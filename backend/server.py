...
def run_rank_diff_socket(data):
    ...
    try:
        ...
        rank_params_csv_path = os.path.join(rank_script_cwd, "rankParameters.csv")
        ...
        logging.debug(f"Executing command: {' '.join(cmd)} in CWD: {rank_script_cwd}")

        process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=rank_script_cwd)
...
ROOT_DIR_LABELS = {
    ...
    'parent_dir_histogram': '6. Histogram', 'parent_dir_input_ds': 'a. Input Dataset Files',
...
def get_explorer_roots():
    ...
    output_folders_order = ['parent_dir_dataset', 'indexes', 'range_query_results', 'trainingSets', 'datasetsAugmentation', 'augmentation_logs', 'parent_dir_histogram']
...
def run_augmentation_socket(data):
    ...
    try:
        ...
        params_file_path = os.path.abspath('augmentationParameters.csv')

...
# Get absolute path to the folder where server.py is
...
def _run_queries_thread(data):
    """
    Runs the Python RangeQuery script in a background thread.
    ...
    These generated queries are then written to the CSV input file for the Python application.
    ...
    try:
        # --- 1. Extract Data and Set Up Paths ---
        ...
        # Set paths for the Python script execution
        current_script_dir = os.path.dirname(os.path.abspath(__file__))
        range_query_script_path = os.path.join(current_script_dir, "RangeQuery.py")
        ...
        # Check for valid dataset files (.csv, .wkt, .geojson)
        ...
        if query_mode == 'random':
            ...
            for q in queries_from_frontend:
                ...
                for _ in range(num_queries):
                    ...
                    processed_queries.append({
                        "datasetName": ds_name, "numQuery": 1, # Specific query, so num=1
                        "queryArea": area, 
            ...
            # Manual Mode: Map frontend fields directly using the expected 'datasetName'
            for q in queries_from_frontend:
                processed_queries.append({
                    "datasetName": q.get('datasetName', ''), "numQuery": q.get('numQuery', ''),
                    "queryArea": q.get('queryArea', ''), "minX": q.get('minX', ''), "minY": q.get('minY', ''),
                    "maxX": q.get('maxX', ''), "maxY": q.get('maxY', ''), "areaint": q.get('areaint', '')
                })

        # --- 3. CSV Creation for Python Script ---
        ...
        # Updated columns format for the Python script
        csv_columns_for_python = ["datasetName", "numQuery", "queryArea", "minX", "minY", "maxX", "maxY", "areaint"]
        with open(query_input_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=csv_columns_for_python, delimiter=';')
            writer.writeheader()
            for row in processed_queries:
                writer.writerow(row)

        # Filter datasets to process based on query definitions (generated or manual)
        datasets_in_query_file = {q.get('datasetName') for q in processed_queries}
        ...
        # --- 4. Subprocess Execution ---
        has_errors = False
        range_params_csv_path = os.path.join(DATA_BASE_PATH, "rangeParameters.csv")
        ...
        for i, dataset_file_name in enumerate(datasets_to_process_actually):
            ...
            # Dynamically create the rangeParameters.csv configuration file
            with open(range_params_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                writer.writeheader()
                writer.writerow({
                    "pathDatasets": parent_dataset_folder_path, "nameDataset": dataset_file_name, # Python requires the file extension
            ...
            for line in process.stdout:
                logging.info(f"Python RangeQuery output ({single_dataset_name_without_ext}): {line.strip()}")
...
def _run_queries_from_csv_thread(data):
    """
    Runs the Python RangeQuery script using an uploaded CSV file in a background thread.
    Handles decoding the uploaded file, matching datasets, and executing the process.
    """
    stop_monitoring_event = threading.Event()
    ...
    try:
        ...
        # --- 2. Decode and save the uploaded CSV file ---
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        unique_id = uuid.uuid4().hex[:8]
        ...
        # Ensure compatibility with both old ('queryDatasetName') and new ('datasetName') CSV headers
        ...
        # Filter datasets to process based on the uploaded query file
        datasets_to_process_actually = [df for df in dataset_files if os.path.splitext(df)[0] in datasets_in_query_file]
        if not datasets_to_process_actually:
            socketio.emit('generate_query_error', {'error': 'None of the datasets in the folder are mentioned in the uploaded file.'})
            return
        
        # --- 3. Parameter Collection and Subprocess Execution ---
        has_errors = False
        range_params_csv_path = os.path.join(DATA_BASE_PATH, "rangeParameters.csv")
        range_params_header = ["pathDatasets", "nameDataset", "pathSummaries", "nameSummary", "pathIndexes", "pathRangeQueries", "nameRangeQueries"]
        total_datasets_to_process = len(datasets_to_process_actually)

        # Collect all dataset parameters for the multi-row CSV required by the Python script
        all_dataset_rows = []
        for dataset_file_name in datasets_to_process_actually:
            ...
            row_data = {
                ...
                "nameDataset": dataset_file_name, # Python script expects the full file name with extension
        ...
        # Write all datasets to rangeParameters.csv in a single operation
        with open(range_params_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=range_params_header, delimiter=';')
            writer.writeheader()
        ...
        # Execute the Python RangeQuery script once
        cmd = ["python", "-u", range_query_script_path]
        ...
        # Parse the Python script's standard output for progress updates
        for line in process.stdout:
            logging.info(f"Python RangeQuery output: {line.strip()}")
            
            # Regex specifically matching the new Python script's log format:
            # e.g., "<System> Starting the range queries process for 'dataset1.csv'!"
            if "Starting the range queries process for" in line:
                try:
                    dataset_name_match = re.search(r"Starting the range queries process for '(.*?)'!", line)
                    if dataset_name_match:
                        full_dataset_name = dataset_name_match.group(1)
                        # Remove the extension for clean UI display
                        dataset_name = os.path.splitext(full_dataset_name)[0]
                        
                        if dataset_name not in datasets_started:
                            datasets_started.add(dataset_name)
                            current_processed_count = len(datasets_started)
                            
                            progress_percentage = int((current_processed_count / total_datasets_to_process) * 99)
                    ...
                    logging.error(f"Error parsing Python progress line: {e}")
...
def run_python_indexing_and_emit(data, stop_monitoring_event):
    """
    Runs the Python indexing process (IndexingSequential.py) in a background thread 
    and emits progress via Socket.IO.
    ...
    # Path setup for the script execution
    current_script_dir = os.path.dirname(os.path.abspath(__file__))
    indexing_script_path = os.path.join(current_script_dir, "Indexing.py")
    ...
    try:
        ...
        # --- 2. Create Configuration CSV ---
        # The CSV is now saved in the same root folder as server.py (DATA_BASE_PATH)
        index_params_csv_path = os.path.join(DATA_BASE_PATH, "indexParameters.csv")
        csv_columns = ["pathDatasets", "nameDataset", "pathIndexes", "typePartition", "num"]
        
        with open(index_params_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        logging.debug(f"indexParameters.csv created at: {index_params_csv_path}")

        # --- 3. Execute Python Subprocess ---
        # We use '-u' to force unbuffered stdout, ensuring real-time progress logs.
        cmd = ["python", "-u", indexing_script_path]
        ...
        socketio.emit('spatial_indexing_progress', {
            'progress': 1, 
            'message': 'Initializing IndexingSequential process...', 
            'processed_count': 0, 
        ...
        for line in process.stdout:
            ...
            logging.info(f"Indexing stdout: {line_stripped}")
            
            # Intercept the end partitioning from Indexing.py
            # Es: [ForkPoolWorker-1] <System> Time for partitioning 'nomefile.csv': 1.2345
            if "<System> Time for partitioning '" in line_stripped:
                try:
                    # Extract the file name from logs
                    ...
                    socketio.emit('spatial_indexing_progress', {
                        'progress': progress, 
                        'file_name': file_name,
                        'message': f"Completed indexing {processed_count}/{total_files}: {file_name}",
                        'processed_count': processed_count, 
                        'total_count': total_files
                    })
                    socketio.sleep(0)
                except (IndexError, ValueError):
                    logging.warning(f"Failed to parse progress line: {line_stripped}")
        
        process.wait()
        
        # --- 5. Handle Final Status ---
        if process.returncode != 0:
            logging.error(f"Error executing IndexingSequential.py. Return code: {process.returncode}")
            socketio.emit('spatial_indexing_error', {'error': f"Indexing failed. Check server logs for details."})
        else:
            logging.info("IndexingSequential.py executed successfully.")
        ...
        # Ensure the monitoring thread is stopped
        stop_monitoring_event.set()
        # Clean up the parameter file
        if os.path.exists(index_params_csv_path):
            try:
                os.remove(index_params_csv_path)
                logging.debug("indexParameters.csv deleted.")
            except Exception as cleanup_error:
                logging.warning(f"Could not delete indexParameters.csv: {cleanup_error}")
...
def process_spatial_index_route():
    ...
    socketio.start_background_task(run_python_indexing_and_emit, data, stop_monitoring_event)
...
def parse_fd_support(filepath):
    """Legge fdSupport.csv e parsa i valori x e y per inviarli al frontend."""
    results = []
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        try:
            ...
            return results  # File vuoto
            
        for row in reader:
            if len(row) < 8: continue
            dim = int(row[6]) if row[6].strip() else 0
            raw_values = row[7:]
            # Divido l'array values a metà in X e Y in base a 'dim'
            x_vals = [float(v) for v in raw_values[:dim] if v.strip()]
            y_vals = [float(v) for v in raw_values[dim:2*dim] if v.strip()]
            
            results.append({
                'parameter': row[0], 'path_nameSummary': row[1], 'datasetName': row[2],
                'path_nameFD': row[3], 'start': row[4], 'end': row[5], 'dim': dim,
                'x': x_vals, 'y': y_vals
            })
    return results

def write_fd_support(filepath, data_list):
    """Sovrascrive fdSupport.csv con i range (start/end) aggiornati dal frontend."""
    header = ['parameter', 'path_nameSummary', 'datasetName', 'path_nameFD', 'start', 'end', 'dim', 'values']
    with open(filepath, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f, delimiter=';')
        writer.writerow(header)
        for item in data_list:
            # Ricostruisco l'array values unendo x e y
            values = item['x'] + item['y']
            row = [
                item.get('parameter', ''), item.get('path_nameSummary', ''), item.get('datasetName', ''),
                item.get('path_nameFD', ''), item.get('start', ''), item.get('end', ''), item.get('dim', '')
            ] + values
            writer.writerow(row)
...
def run_fractal_dimension_socket(data):
    """
    Fase 1: Prepara i parametri e lancia FractalDimension.py per calcolare i valori intermedi (X e Y).
    """
    ...
    try:
        ...
        fd_support_path = os.path.join(DATA_BASE_PATH, 'fdSupport.csv')
        support_header = ['parameter', 'path_nameSummary', 'datasetName', 'path_nameFD', 'start', 'end', 'dim', 'values']
        
        with open(fd_support_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f, delimiter=';')
            writer.writerow(support_header)
            # Creiamo una riga "vuota" pre-impostata per ogni parametro scelto dall'utente
    ...
    # --- 4. Handle Script Results ---
        if process.returncode != 0:
            # Catturiamo il crash di Python sui dataset di punti (non bounding box)
            if "TypeError: float() argument" in full_output and "'NoneType'" in full_output:
                custom_error = "Errore: Il dataset selezionato non sembra essere formato da Bounding Box (4 coordinate). I punti (2 coordinate) non sono supportati."
                logging.error(custom_error)
                emit('fractal_dimension_error', {'error': custom_error, 'details': full_output})
            else:
            ...
            logging.info(f"FractalDimension.py Phase 1 executed successfully.")
            
            # Se la scelta è 'distribution', il nuovo Python ha già fatto tutto da solo.
            if 'distribution' in parameters_list:
                emit('fractal_dimension_complete', {'message': 'Fractal dimension calculated successfully!'})
            else:
                # Per tutti gli altri parametri, leggiamo i dati intermedi generati dallo script
                fd_support_path = os.path.join(DATA_BASE_PATH, 'fdSupport.csv')
                intermediate_data = parse_fd_support(fd_support_path)
                
                # Filtriamo via lo "scheletro" vuoto per non far impazzire i grafici React
                valid_data = [d for d in intermediate_data if len(d['x']) > 0 and len(d['y']) > 0]
                
                if valid_data:
                    emit('fractal_dimension_intermediate', {'data': valid_data})
                else:
                    emit('fractal_dimension_error', {'error': 'Nessun dato intermedio valido generato in fdSupport.csv.'})
...
@socketio.on('calculate_final_fd')
def calculate_final_fd_socket(data):
    """
    Fase 2: Riceve i dati con start/end aggiornati dall'utente e lancia il calcolo finale della pendenza.
    """
    stop_monitoring_event = threading.Event()
    try:
        updated_data = data.get('intermediateData', [])
        if not updated_data:
            emit('fractal_dimension_error', {'error': 'No data provided for final calculation.'})
            return

        # 1. Aggiorna fdSupport.csv con le scelte dell'utente
        fd_support_path = os.path.join(DATA_BASE_PATH, 'fdSupport.csv')
        write_fd_support(fd_support_path, updated_data)

        # 2. Modifica fdParameters.csv per impostare parameters a 'fd' (calcolo pendenza)
        fd_params_path = os.path.join(DATA_BASE_PATH, 'fdParameters.csv')
        with open(fd_params_path, 'r', encoding='utf-8') as f:
            reader = list(csv.reader(f, delimiter=';'))
            header = reader[0]
            row_data = reader[1]
        
        # Mantiene tutti i percorsi, ma sovrascrive i parametri passati alla fine con "fd"
        row_data = row_data[:9] + ['fd'] 

        with open(fd_params_path, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f, delimiter=';')
            writer.writerow(header)
            writer.writerow(row_data)

        # 3. Lancia nuovamente lo script Python (NOME AGGIORNATO)
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "FractalDimension.py")
        cmd = ["python", "-u", script_path]
        ...
        if process.returncode != 0:
            emit('fractal_dimension_error', {'error': "Final calculation script failed."})
        else:
            emit('fractal_dimension_complete', {'message': 'Fractal dimension successfully calculated!'})

    except Exception as e:
        logging.exception("Exception during final FD calculation.")
        emit('fractal_dimension_error', {'error': f'An internal server error occurred: {str(e)}'})
    finally:
        stop_monitoring_event.set()

...
def rotate_dataset(dataset_name, pathIndexes, output_ds, degree):
	...
	master_table = os.path.join(partitions_folder,"master_table.csv")				# percorso contenente la Master Table correlata all'indice spaziale di "dataset_name"
	...
	partitions = df_master['NamePartition'].tolist()								# lista delle partizioni di "dataset_name"
...
def update_range_query_file(index, rotated_dataset_name, minX, minY, maxX, maxY, path_nameRangeQueriesResult):
	...
	new_row = {
		...
		'averageExecutionTime': original_row['averageExecutionTime'],
		'numberParallelThreads': original_row['numberParallelThreads'],
		'totalExecutionTime': original_row['totalExecutionTime'],
...
def update_rq_merge(rq2_row, dsNew_name, dsNew_cardinality, label, path_nameRangeQueriesResult):
	...
	new_row = {
		...
		'averageExecutionTime': rq2_row['averageExecutionTime'].iloc[0],
		'numberParallelThreads': rq2_row['numberParallelThreads'].iloc[0],
		'totalExecutionTime': rq2_row['totalExecutionTime'].iloc[0],
		last_column_name: label
	}
...
def main():
	...
	# Identifico eventuali dataset che possono essere uniti con la tecnica del merge
				...
				# SECONDA TECNICA DI AUGMENTATION - "NOISE"
					...
					try:
						...
						if not noise_data.empty:
							...
							file_path = os.path.join(pathIndexes,f'{dataset_name}_spatialIndex/master_table.csv')	# caricamento cartella contenente la tabella ricapitolativa sull'indice spaziale correlato al dataset in questione

							# controllo esistenza dell'indice spaziale
							try:
								df = pd.read_csv(file_path, sep=',')								# tentativo di apertura di "file_path"
							except FileNotFoundError:												# se il file non viene aperto correttamente, stampa messaggio di errore
								print("File not found. Exiting.")
							
							df = pd.read_csv(file_path, sep=',')									# lettura della tabella ricapitolativa dell'indice spaziale
							...
							name_df = df[['NamePartition']]											# estrazione dei nomi di ciascuna partizione del dataset in questione														

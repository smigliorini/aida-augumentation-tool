...
# 		 cardinality --> 
# 		 path_nameNewDatasets --> percorso contenente i nuovi datasets generati
def update_range_query_param(index, new_dataset_name, cardinality, path_nameRangeQueriesResult):
	df_range_query = pd.read_csv(path_nameRangeQueriesResult, delimiter=';')							# lettura del file con le range queries già esistenti
	original_row = df_range_query.iloc[index]															# ricerca della range query in questione sfruttando l'indice
		...
		# CASO 3: il numero di queries richiesto è maggiore al numero di queries presenti nel bin selezionato - INCREMENTO QUERIES
			...
			while True:
				...
				"""
					...
					try:
						merge_data = main_data																# range queries
						...
						# Controllo che, dopo il filtro, ci siano almeno due dataset diversi (non potrei fare il merge avendo un solo dataset)
						if len(sampled_groups) < 2:
							raise ValueError("Not enough unique dataset names to sample from.")

						# Cerco due datasets che siano disgiunti tra loro e li unisco
						tent = 0
						while True:
							while True:
								print("<System> Take two datasets!")
								random_rows = sampled_groups.sample(n=2, replace=False)								# pesco due righe (due dataset diversi)
								dataset_names = random_rows['datasetName'].tolist()									# estraggo i due nomi dei datasets
								summary_data_rows = summary_data[summary_data['datasetName'].isin(dataset_names)]	# seleziono le righe dei due dataset dal sommario
								tent = tent + 1
								print (tent)

								# estrazione delle dimensioni delle finestre di dataset (x1, y1, x2, y2) di ciascun dataset selezionato
								row1 = summary_data_rows.iloc[0][['x1', 'y1', 'x2', 'y2']].tolist()					# dataset_1
								row2 = summary_data_rows.iloc[1][['x1', 'y1', 'x2', 'y2']].tolist()					# dataset_2
						
								# verifica che i due datasets siano disgiutni
								print("<System> Verify if two datastes are disjoint.")
								if are_disjoint(row1, row2):
									break
						
								# se dopo il ventesimo tentativo non ho trovato due dataset disgiunti, rinuncio e devo provare con un'altra tecnica
								if tent == 20:
									print("<System> 20 attempts were made to find two disjoint datasets! Proceeding with another tecnique.")
									num_queries_inserted -=1
									break
							...
							# Parametri utili per l'unione dei due datasets
							name_d1 = summary_data_rows.iloc[0]['datasetName']		# nome del dataset_1
							name_d2 = summary_data_rows.iloc[1]['datasetName']		# nome del dataset_2
							nf_d1 = summary_data_rows.iloc[0]['num_features']		# numero di geometrie nel dataset_1
							nf_d2 = summary_data_rows.iloc[1]['num_features']		# numero di giometrie nel dataset_2
							sum_nf = nf_d1 + nf_d2									# numero di geometrie totali nel nuovo dataset
							card_d1 = random_rows.iloc[0]['cardinality']			# cardinalità della query riferita al dataset_1
							card_d2 = random_rows.iloc[1]['cardinality']			# cardinalità della query riferita al dataset_2
							random_row = random_rows.sample(n=1).iloc[0]			# seleziono una delle due query scelte in precedenza (da cui "ereditare" la cardinalità)
						
							if random_row.equals(random_rows.iloc[0]):				# se è stata selezionata la prima query (riferita a dataset_1) ...
								inside = card_d1 * nf_d1							# ... calcolo il numero di geometrie all'interno della query uno
							else:													# se è stata selezionata la seconda query (riferita a dataset_2) ...
								inside = card_d2 * nf_d2							# ... calcolo il numero di geometrie all'interno della query due
						
							new_card = inside/sum_nf								# calcolo della nuova cardinalità (geometrie_query/geometrie_dataset)

							# se il parametro da categorizzare è la cardinalità, devo verificare se la nuova cardinalità entra nel bin corretto
							if param_to_categorize == "cardinality":
								# lavoro per individuare i limiti del bin in questione
								bin_label = row['cardinality_class']
								bounds = bin_label.split('-')
								lower_bound = float(bounds[0])
								upper_bound = float(bounds[1])

								# se la nuova cardinalità è compresa nel bin, proseguo
								if lower_bound <= new_card <= upper_bound:
									break

								# altrimenti devo rifare il ragionamento
								else:
									print("<System> New cardinality was not in the bin. Nuovo tentativo in corso...")
									print(f"<System> Tentativo: {tent}")
									tent = tent + 1
									continue
							
							# se il parametro da categorizzare è un'altro, proseguo
							else: 
								break
						
						# proseguo solo se ho unito effettivamente i due dataset in meno di 20 tentativi
						if tent < 20:								
							# lettura del dataset_1
							in1 = f"{pathDatasets}/{name_d1}.csv"
							dataset1 = pd.read_csv(in1, sep=',', header=None)
							
							# lettura del dataset_2
							in2 = f"{pathDatasets}/{name_d2}.csv"
							dataset2 = pd.read_csv(in2, sep=',', header=None)
						
							# composizione del nome del nuovo dataset combinato
							n1, n2 = extract_numbers(name_d1), extract_numbers(name_d2)					# numero dataset_1 e dataset_2
							combined_filename = f"{path_nameNewDatasets}/dataset_{n1[0]}_{n2[0]}_combined.csv"
							comb = f"dataset_{n1[0]}_{n2[0]}_combined"

							combined_dataset = pd.concat([dataset1, dataset2], ignore_index=True)		# concatenazione dei due datasets
							combined_dataset.to_csv(combined_filename, index=False, header=False)		# salvataggio in un file ".csv"
															
							update_dataset_param(random_row['datasetName'], comb, sum_nf, path_nameSummary, path_nameNewDatasets)
							if param_to_categorize == "cardinality":
								update_range_query_param(file_index, comb, new_card)
							else:
								update_range_query_param2(file_index, comb, new_card, bin_label, path_nameRangeQueriesResult)
								
							print("<System> The 'merge' operation for the following query has finished")

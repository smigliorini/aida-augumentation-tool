...
def csvReading(filePath):
	...
	Output: parameterCategorized --> parametro da categorizzare;
...
def searchMax(fileName):
	...
	"""
	Funzione che restituisce i valori massimi dei campi 'cardinality', 'mbrTests' e 'totalExecutionTime'.
	Input:  fileName --> percorso completo del file contenente i parametri risultanti per ciascuna query.
	Output: maxValues --> valori massimi trovati per ciascun parametro.

	"""
	
	searchColumn = ["cardinality", "mbrTests", "totalExecutionTime"]			# Colonne da analizzare all'interno del file .csv
...
def create_intervals(min_val, max_val, interval_points):
	...
	INPUT:  min_val --> es. 0;
			max_val --> es. 40;
			interval_points --> es. [10, 20, 30].
	OUTPUT: bins --> es. [0, 10, 20, 30, 40];
			labels --> es. ['0.0-10.0', '10.0-20.0', '20.0-30.0', '30.0-40.0'].
	"""
...
def main():
	...
	# -------------------------------------------------------------------------------------------------------------------------------------------------------
	# LETTURA DEL FILE 'binParameters.csv' E SALVATAGGIO DEGLI INPUT:
	...
	# -------------------------------------------------------------------------------------------------------------------------------------------------------
	# LAVORAZIONE SUI PARAMETRI 'cardinality', 'mbrTests', 'totalExecutionTime':
	# Valori minimi per parametro (cardinality, mbrTests, totalExecutionTime)
	min_values = {
		'cardinality': 0.0,  
		'mbrTests': 0 ,
	...
	# Verifica che il parametro passato sia uno tra "cardinality", "mbrTests" e "totalExecutionTime"
	...
	# Valori massimi per parametro (cardinality, mbrTests, totalExecutionTime) - da ricavare dal file in "nameRangeQueriesResult"
	print("<System> Finding maximum values for parameters!")
	path_rangeQueriesResult = os.path.join(pathRangeQueriesResult, nameRangeQueriesResult)		# Percorso completo contenente il file 'nameRangeQueriesResult'
	if not os.path.exists(path_rangeQueriesResult):												# Verifica esistenza del file
		raise ValueError(f"<System> ERROR! The file '{nameRangeQueriesResult}' does not exist in '{pathRangeQueriesResult}'...")
	max_values = searchMax(path_rangeQueriesResult)												# Individuazione dei valori massimi richiesti
	# -------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	valid_fd_files = [f for f in nameFD if f.strip()]  # rimuove stringhe vuote
	if valid_fd_files:
		for file in valid_fd_files:

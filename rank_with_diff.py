...
def main():
	#rqResult = 'rq_result.csv'						# file contenente i risultati delle range queries
	#datasetSummaries = 'dataset-summaries.csv'		# file contenente i dataset con rispettivi parametri di generazione
	#fdGeom = 'fd2_geom_allds.csv'					# file ...
	#param_to_categorize --> parameterCategorized
	#numberInterval --> numberIntervals
	...
	max_index = 0
	pathFolderOutput = f"trainingSets/{os.path.basename(pathRangeQueriesResult)}/{nameRangeQueriesResult.replace('rqR_', '').replace('.csv', '')}"
	...
	main_data.head(0).to_csv(newPathDiff, index=False, sep=';')
	
	# 2. Inserimento di "dataset-summaries_ts.csv" e "dataset-summaries_diff.csv"
	...
	with open(f"{pathSummaries}/{nameSummary}", "r", encoding="utf-8") as input, \
		...
		for i, line in enumerate(input):
			...
			elif line.startswith(dataset):		# linea dataset --> solo in Ts
				outputTs.write(line)
